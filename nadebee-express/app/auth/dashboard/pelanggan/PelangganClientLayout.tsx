...
import { notFound } from "next/navigation"; // <-- Import Satpam 404
...
export default function PelangganLayout({
  ...
  const [isKurir, setIsKurir] = useState<boolean | null>(null); // <-- State Satpam

  useEffect(() => {
    // Kurir meninggalkan jejak 'loggedInCourierId' di browser
    const courierId = sessionStorage.getItem("loggedInCourierId");
    setIsKurir(!!courierId);
  }, []);

  // CASE 2: Kalau dia Kurir tapi maksa masuk root Pelanggan -> 404 Not Found
  if (isKurir === true) {
    notFound(); 
  }

  // Mencegah layar berkedip saat sedang mengecek
